...
function MenuEditor(idSelector, options) {
    ...
    $main.on('click', '.btnUp', function (e) {
        e.preventDefault();
        var $li = $(this).closest('li');
        var myOrdering = $li.data().ordering;
        var prevOrdering = $li.prev('li').data().ordering;
        if(myOrdering == prevOrdering){
            myOrdering+=1;
        }
        console.log('prev_ording',prevOrdering);
        console.log('current_ording',myOrdering);
        $li.data('ordering',prevOrdering);
        $li.prev('li').data('ordering',myOrdering);
    ...
    $main.on('click', '.btnDown', function (e) {
        ...
        var $li = $(this).closest('li');
        var myOrdering = $li.data().ordering;
        var nextOrdering = $li.next('li').data().ordering;
        if(myOrdering == nextOrdering){
            myOrdering+=1;
        }
        console.log('next_ording',nextOrdering);
        console.log('current_ording',myOrdering);
        $li.data('ordering',nextOrdering);
        $li.next('li').data('ordering',myOrdering);
        $li.next('li').after($li);
        MenuEditor.updateButtons($main);
    });
    $main.on('click', '.btnOut', function (e) {
        ...
        $li.data().parent_id = $liParent.data().parent_id;
        console.log($li.data());
        $liParent.after($li);
    ...
    $main.on('click', '.btnIn', function (e) {
        ...
        $li.data().parent_id = $prev.data().id;
        console.log($li.data());
        if ($prev.length > 0) {
    ...
    function editItem($item) {
        ...
        if(data.hasOwnProperty('icon'))
            $form.find("img").attr("src",$('#url').val() + data.icon);
    ...
    this.update = function(data=null){
        ...
        if(data){
            $cEl.data("name",data.name);
            $cEl.data("text",data.text);
            $cEl.data("home",data.home?1:0);
            $cEl.data("active",data.active?1:0);
            $cEl.data("icon",data.icon);
        }
